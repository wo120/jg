...
class Culture extends BaseController
{
    public function lists()
    {
        ...
        if (!empty($type) && empty($title)) {
            $culture->where('type', '=', $type);
            $this->assign('type', $type);
            $pageParam['query']['type'] = $type;
        }

        if (empty($type) && !empty($title)) {
            $culture->where('title', '=', $title);
            $this->assign('title', $title);
            $pageParam['query']['title'] = $title;
        }

        $list = $culture->order('id desc')->paginate(20,false,['query'=>request()->param()])->each(function($item, $key){
            $base_url = config('base_url');
            $base_url = $base_url['url'];
            $item['img'] = $base_url.'/'.$item["img"]; //给数据集追加字段num并赋值
            return $item;
        });
    ...
    public function add_go()
    {
        ...
        $res = Db::table('culture')->insert($arr);
    ...
    public function edit()
    {
        ...
        $data = Db::table('culture')->where('id',$id)->find();
    ...
    public function edit_go()
    {
        ...
        $res = Db::table('culture')->where('id',$id)->update($arr);
    ...
    public function edit_up(Request $request)
    {
        ...
        if($file){
            ...
            if($info){
                ...
                if ($imgs){
                    $res = Db::table('culture')->where('id',$id)->update(['img'=>$img]);
                    $url = config('base_url');
                    $base_url = $url['url'];
    ...
    public function del()
    {
        ...
        $res = Db::table('culture')->where('id',$id)->delete();
        return $res?1:0;
    }
    //批量删除
    public function dels()
    {
        ...
        $ids = $data['ids'];
//        dd($ids);
        $res  = Db::table('culture')->where('id','in',$ids)->delete();
        return $res? 1 :  0;
    }
