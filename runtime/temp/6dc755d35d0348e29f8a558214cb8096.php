...
<div class="x-body">
    <div class="layui-row">
        <form class="layui-form layui-col-md12 x-so">
            ...
            <!--<input class="layui-input" placeholder="标题" name="start_time" id="start">-->

            <div class="layui-input-inline">
                <select name="type">
                    <option>期数</option>
                    <?php if(is_array($npers) || $npers instanceof \think\Collection || $npers instanceof \think\Paginator): $i = 0; $__LIST__ = $npers;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$n): $mod = ($i % 2 );++$i;?>
                    <option value="<?php echo $n['nper']; ?>"><?php echo $n['nper']; ?></option>
                    <?php endforeach; endif; else: echo "" ;endif; ?>
                </select>
            </div>
    ...
    <xblock>
        ...
        <button class="layui-btn" onclick='x_admin_show("添加新闻报","<?php echo url('admin/news/cc_add'); ?>")'><i class="layui-icon"></i>添加</button>
    ...
    <table class="layui-table">
        ...
        <tr>
            ...
            <!--name  start   end    is_top   type  线上/线下  content 活动内容 is_display  是否显示  is_top 停止  is_need_money money
活动封面     活动名称     活动时间       活动是否免费      活动价格      是否显示   是否停止             -->
            ...
            <td class="td-manage">
                <a title="查参加活动的人员"  onclick='x_admin_show("查看","<?php echo url('admin/activity/show'); ?>?id=<?php echo $v['id']; ?>")' href="javascript:;">
                <i class="layui-icon">&#xe63c;</i>

                </a>
                <a title="编辑"  onclick='x_admin_show("编辑","<?php echo url('admin/news/cc_edit'); ?>?id=<?php echo $v['id']; ?>")' href="javascript:;">
                <i class="layui-icon">&#xe642;</i>
                </a>
...
<script>
    ...
    function cc_news_del(obj,id){
        layer.confirm('确认要删除吗？',function(index){
            ...
            $.ajax({
                url:'<?php echo url("admin/news/cc_news_del"); ?>',
                type:'POST',
                data:{"id":id},
    ...
    function delAll (argument) {
        ...
        layer.confirm('确认要删除吗？'+data,function(index){
            ...
            console.log(data);
            $.ajax({
                url:'<?php echo url("admin/news/cc_dels"); ?>',
                type:'POST',
                data:{"ids":data},
                success:function(data){
                    console.log(data);
